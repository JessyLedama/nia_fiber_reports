...
class StockAdjustmentReport(models.TransientModel):
    ...
    def check_excel_report(self):
        ...
        products = self.env['stock.quant'].search_read([('create_date', '>=', startDate), ('create_date', '<=', endDate)]) 
    ...
    def print_excel_report(self, data):
        return self.env.ref('nia_fiber_reports.action_stock_adjustment_report_xlsx').report_action(self, data=data, config=False)
