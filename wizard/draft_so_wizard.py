...
class DraftSOReport(models.TransientModel):
    _name = "draft.so.report"
    _description = "Draft SO Report"
    ...
    def check_excel_report(self):
        ...
        products = self.env['sale.order.line'].search_read([('state', '=', 'draft')]) 
    ...
    def print_excel_report(self, data):
        return self.env.ref('nia_fiber_reports.action_draft_so_report_xlsx').report_action(self, data=data, config=False)
